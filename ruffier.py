...
def neud_level(age):
    ...
    norm_age = (min(age, 15) - 7) // 2 #каждые 2 года разницы от 7 лет превращаются в единицу - вплоть до 15
    result = 21 - norm_age * 1.5 #умножаем каждые 2 года разницф нв 1.5< так распределим уровни в таблице
    return result
...
def ruffier_result(r_index, level):
    ...
    if r_index >= level:
        return 0
    level = level - 4 #это будет выполняться, если мы уже вернули ответ 'неуд'
    if r_index >= level:
        return 1
    ...
    if r_index >= level:
        return 3
    return 4 #здесь оказались, если индекс меньше всех промяжуточных уровней, т.e. тестируемый крут.
...
def test(P1, P2, P3, age):
    ...
    if age < 7:
        return (txt_index + '0', txt_nodata) #тайеа сия не для текста сего
    else:
        ruff_index = ruffier_index(P1,P2,P3)#расчет
        result = txt_res[ruffier_result(ruff_index, neud_level(age))] #интерпритация, перевод числового уровня подготовки в текстовые данные
