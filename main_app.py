...
def check_int(str_num):
    #возвращает число или  False, если строка не конвентируется
    try:
        return int(str_num)
    except:
        return False
...
class InstrScr(Screen):
    def __init__(self, **kwargs):
        ...
        self.in_name = TextInput(multilane = False)
    ...
    def next(self):
        ...
        age = chaeck_int(self.in_age.text)
...
class PulseScr(Screen):
    def __init__(self, **kwargs):
        ...
        #lbl1 = Label(text = 'Считай пульс')
        self.lbl_sec = Seconds(15)
        self.lbl_sec.bind(done = self.sec_finished)

        line = BoxLayout(size_hint(0.8, None), halign = '30sp')
        lbl_result = Label(text = 'Введите резултат', halign = 'right')
...
class PulseScr2(Screen):
    def __init__(self, **kwargs):
            super().__init__(**kwargs)
            instr = Label(text = txt_test_3)
            line1 = BoxLayout(size_hint(0.8, None), height = '30sp')
            lbl_result1 = Label(text = 'Результат', halign = 'right')
            self.in_result = TextInput(text = '0', multiline = False)
            line1.add_widget(lbl_result1)
            line1.add_widget(self.in_result)
            line2 = BoxLayout(size_hint =(0.8, None), halign ='30sp')
            lbl_result2 = Label(tezt = 'Результат после отдыха:', halign ='right')
